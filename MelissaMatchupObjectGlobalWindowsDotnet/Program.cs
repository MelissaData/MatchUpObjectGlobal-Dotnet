...
namespace MelissaMatchUpObjectGlobalWindowsDotnet
{
  /// <summary>
  ...
  ///   a generated C# wrapper over mdMatchup.dll and mdGlobalParse.dll that the accompanying
  ///   MelissaMatchupObjectGlobalWindowsDotnet.ps1 script downloads on every run.
  ///
  /// Reference:
  ...
  class Program
  {
    ...
    /// <param name="testGlobalFile">Receives the path to the global input file to test in one-shot mode.</param>
    /// <param name="testUsFile">Receives the path to the US input file to test in one-shot mode.</param>
    /// <param name="dataPath">Receives the path to the MatchUp Object data files.</param>
    ...
    /// <param name="testGlobalFile">A global input file to process in one-shot mode; if empty, the program prompts interactively.</param>
    /// <param name="testUsFile">A US input file to process in one-shot mode.</param>
    /// <param name="dataPath">Path to the MatchUp Object data files.</param>
    static void RunAsConsole(string license, string testGlobalFile, string testUsFile, string dataPath)
    {
      Console.WriteLine("\n\n========== WELCOME TO MELISSA MATCHUP OBJECT GLOBAL WINDOWS DOTNET ==========\n");
  ...
  {
    // Input: the global input file to deduplicate.
    public string InputFilePath1   { get; set; } = "";

    // Input: the US input file to deduplicate.
    public string InputFilePath2 { get; set; } = "";

    // Output: file the deduplicated global records are written to.
    public string OutputFilePath1  { get; set; } = "";

    // Output: file the deduplicated US records are written to.
    public string OutputFilePath2 { get; set; } = "";

    // Result codes are written per record into the output files rather than collected
    // here, so this sample never assigns this property.
